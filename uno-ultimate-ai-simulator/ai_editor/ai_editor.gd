...
func _input(event):
	# 1. Spara (Ctrl+S) - Kräver fortfarande specifik knappkoll eftersom det är ett kommando
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_S and event.is_command_or_control_pressed():
			_on_save_button_pressed()
			get_viewport().set_input_as_handled()
			
	# 2. Radera noder (Nu via vår skottsäkra Input Map!)
	# "is_action_pressed" fångar upp både Backspace och Delete automatiskt
	if event.is_action_pressed("delete_node"):
		var nodes_to_delete: Array[StringName] = []
		
		# Leta igenom duken efter noder som användaren har markerat
		for child in graph_edit.get_children():
